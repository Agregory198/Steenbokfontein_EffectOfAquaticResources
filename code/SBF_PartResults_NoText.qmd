...
dir <- df |>
  ...
  mutate(total = Core + Flake + RetouchedTool,
         ...
         y = cbind(Core_prop, Flake_prop, RetouchedTool_prop),
         )
...
posterior <- posterior_epred(bayes_dm)

# Output corresponds to [chains, RAWMATERIAL (Quartz, Quartzite, Sil), ARTIFACTCLASS (C, F, RT)]
dimnames(posterior)[[2]] <- paste(dir$RAWMATERIAL, dir$STRAT_mod, sep = "_")
dimnames(posterior)[[3]] <- c("Core", "Flake", "Retouched Tool")
# Example: compare Flake proportions between Quartz (row 1) and Quartzite (row 2)
tab <- list()
result <- list()
for(i in 1:length(dimnames(posterior)[[2]])) {
  
  for(j in 1:length(dimnames(posterior)[[3]])){
    
    if(j == 3){
      
      j = 2
      
      name <- paste(dimnames(posterior)[[2]][i], dimnames(posterior)[[3]][1],
                  dimnames(posterior)[[2]][i], dimnames(posterior)[[3]][j+1])
      
      diff <- posterior[, i, 1] - posterior[, i, j+1]
      
      tab[[name]] <- round(diff, 3)
      result[[name]] <- mean(diff > 0)
      
    } else{
      
      name <- paste(dimnames(posterior)[[2]][i], dimnames(posterior)[[3]][j],
                    dimnames(posterior)[[2]][i], dimnames(posterior)[[3]][j+1])
      
      diff <- posterior[, i, j] - posterior[, i, j+1]
...
dir <- df |>
  group_by(RAWMATERIAL, ARTIFACTCLASS, STRAT) |>
  count() |>
  pivot_wider(names_from = ARTIFACTCLASS, values_from = n, values_fill = 0) |>
  mutate(total = Core + Flake + RetouchedTool,
         ...
         Core_propK = (Core + 0.5) / (total + 0.5*K),
         Flake_propK = (Flake + 0.5) / (total + 0.5*K),
         RetouchedTool_propK = (RetouchedTool + 0.5) / (total + 0.5*K),
         
         y = cbind(Core_propK, Flake_propK, RetouchedTool_propK)
         ) |>
  select(RAWMATERIAL, STRAT,
...
# Output corresponds to [chains, RAWMATERIAL (Quartz, Quartzite, Sil), ARTIFACTCLASS (C, F, RT)]
dimnames(posterior2)[[2]] <- paste(dir$RAWMATERIAL, dir$STRAT, sep = "_")
...
for(i in 1:length(dimnames(posterior2)[[2]])) {
  ...
  for(j in 1:length(dimnames(posterior2)[[3]])){
    ...
    if(j == 3){
      ...
      name <- paste(dimnames(posterior2)[[2]][i], dimnames(posterior2)[[3]][1],
                  dimnames(posterior2)[[2]][i], dimnames(posterior2)[[3]][j+1])
      
      diff <- posterior2[, i, 1] - posterior2[, i, j+1]
      ...
      name <- paste(dimnames(posterior2)[[2]][i], dimnames(posterior2)[[3]][j],
                    dimnames(posterior2)[[2]][i], dimnames(posterior2)[[3]][j+1])
      
      diff <- posterior2[, i, j] - posterior2[, i, j+1]
...
##### Flake termination and initiation

```{r}
df.flakes |>
  group_by(STRAT, RAWMATERIAL) |>
  mutate(total = n()) |>
  group_by(STRAT, RAWMATERIAL, FLAKETERMINATION, total) |>
  count() |>
  summarize(prop = n/total) |>
  
  ggplot(aes(x = STRAT, y = prop, fill = factor(FLAKETERMINATION)))+
  ...
  theme(axis.text.x = 
          element_text(colour = 
                         c(rep("skyblue",3),
                           rep("darkmagenta",2),
                           rep("forestgreen", 1)),
                       angle = 45, hjust = 1)) -> flake_term1

df.flakes |>
  group_by(STRAT_mod, RAWMATERIAL) |>
  mutate(total = n()) |>
  group_by(STRAT_mod, RAWMATERIAL, FLAKETERMINATION, total) |>
  count() |>
  summarize(prop = n/total) |>
  
  ggplot(aes(x = STRAT_mod, y = prop, fill = factor(FLAKETERMINATION)))+
  geom_bar(stat = "identity") +
  facet_wrap(~RAWMATERIAL) +
...
df.flakes |>
  group_by(STRAT, RAWMATERIAL) |>
  mutate(total = n()) |>
  group_by(STRAT, RAWMATERIAL, FLAKEINITIATION, total) |>
  count() |>
  summarize(prop = n/total) |>
  
  ggplot(aes(x = STRAT, y = prop, fill = factor(FLAKEINITIATION)))+
  geom_bar(stat = "identity") +
  facet_wrap(~RAWMATERIAL) +
  ...
  theme(axis.text.x = 
          element_text(colour = 
                         c(rep("skyblue",3),
                           rep("darkmagenta",2),
                           rep("forestgreen", 1)),
                       angle = 45, hjust = 1)) -> flake_init1


df.flakes |>
  group_by(STRAT_mod, RAWMATERIAL) |>
  mutate(total = n()) |>
  group_by(STRAT_mod, RAWMATERIAL, FLAKEINITIATION, total) |>
  count() |>
  summarize(prop = n/total) |>
  
  ggplot(aes(x = STRAT_mod, y = prop, fill = factor(FLAKEINITIATION)))+
  geom_bar(stat = "identity") +
  facet_wrap(~RAWMATERIAL) +
  ...
  theme(axis.text.x = 
          element_text(colour = 
                         c(rep("skyblue",1),
                           rep("darkmagenta",1),
                           rep("forestgreen", 1)),
                       angle = 45, hjust = 1)) -> flake_init2
  
```

```{r}
png("../results/figures/fig_FlakeTerm.png", width = 290, height = 120, units = "mm", res = 300)
ggarrange(
  flake_term2, flake_term1,
  align = "h", labels = c("A", "B"),
  common.legend = TRUE
)
...
```{r}
png("../results/figures/fig_FlakeInit.png", width = 290, height = 120, units = "mm", res = 300)
ggarrange(
  flake_init2, flake_init1,
  align = "h", labels = c("A", "B"),
  common.legend = TRUE
)
...
df.flakes |>
  filter(PLATFORM == "Yes") |>
  mutate(PLATFORMPREP = ifelse(PLATFORMPREP == "Full grooves through platform", "FacetWithBulb",
                        ifelse(PLATFORMPREP == "Residual grooves on platform",
                                                   "ResidualFacetWithBulb",
                        ifelse(PLATFORMPREP == "No prepared", "NotPrepared",
                               "Indeterminate")))) |>
  group_by(STRAT, RAWMATERIAL) |>
  mutate(total = n()) |>
  group_by(STRAT, RAWMATERIAL, PLATFORMPREP, total) |>
  count() |>
  na.omit() |>
  summarize(prop = n/total) |>
  
  ggplot(aes(x = STRAT, y = prop, fill = factor(PLATFORMPREP)))+
  geom_bar(stat = "identity") +
  facet_wrap(~RAWMATERIAL) +
...
df.flakes |>
  ...
  group_by(STRAT_mod, RAWMATERIAL) |>
  mutate(total = n()) |>
  group_by(STRAT_mod, RAWMATERIAL, PLATFORMPREP, total) |>
  count() |>
  na.omit() |>
  summarize(prop = n/total) |>
  
  ggplot(aes(x = STRAT_mod, y = prop, fill = factor(PLATFORMPREP)))+
  geom_bar(stat = "identity") +
  facet_wrap(~RAWMATERIAL) +
...
df.flakes |>
  ...
  group_by(STRAT, RAWMATERIAL) |>
  mutate(total = n()) |>
  group_by(STRAT, RAWMATERIAL, PLATABRASION, total) |>
  count() |>
  na.omit() |>
  summarize(prop = n/total) |>
  
  ggplot(aes(x = STRAT, y = prop, fill = factor(PLATABRASION)))+
  geom_bar(stat = "identity") +
  facet_wrap(~RAWMATERIAL) +
...
df.flakes |>
  ...
  ggplot(aes(x = STRAT_mod, y = prop, fill = factor(PLATABRASION)))+
  geom_bar(stat = "identity") +
  facet_wrap(~RAWMATERIAL) +
  ...
  theme(axis.text.x = 
          element_text(colour = 
                         c(rep("skyblue",1),
                           rep("darkmagenta",1),
                           rep("forestgreen", 1)),
                       angle = 45, hjust = 1)) -> plat_ab2
```

```{r}
png("../results/figures/fig_PlatPrep.png", width = 290, height = 120, units = "mm", res = 300)
ggarrange(
  plat_prep2, plat_prep1,
  align = "h", labels = c("A", "B"),
  common.legend = TRUE
)
...
```{r}
png("../results/figures/fig_PlatAb.png", width = 290, height = 120, units = "mm", res = 300)
ggarrange(
  plat_ab2, plat_ab1,
  align = "h", labels = c("A", "B"),
  common.legend = TRUE
...
```{r}
df.flakes |>
  filter(Transport_small < 1) |>
  ggplot(aes(x = STRAT, y = Transport_small))+
  geom_boxplot()+
  facet_wrap(~RAWMATERIAL) +
  ...
  theme(axis.text.x = 
          element_text(colour = 
                         c(rep("skyblue",3),
                           rep("darkmagenta",2),
                           rep("forestgreen", 1)),
                       angle = 45, hjust = 1)) -> flake_trans1



df.flakes |>
  filter(Transport_small < 1) |>
  ggplot(aes(x = STRAT_mod, y = Transport_small))+
  geom_boxplot()+
  facet_wrap(~RAWMATERIAL) +
  ...
  theme(axis.text.x = 
          element_text(colour = 
                         c(rep("skyblue",1),
                           rep("darkmagenta",1),
                           rep("forestgreen", 1)),
                       angle = 45, hjust = 1)) -> flake_trans2

```

```{r}
png("../results/figures/fig_FlakeTrans.png", width = 290, height = 120, units = "mm", res = 300)
ggarrange(
  flake_trans2, flake_trans1,
  align = "h", labels = c("A", "B"),
  common.legend = TRUE
)
...
```{r}
df.flakes |>
  ggplot(aes(x = STRAT, y = PLATFORMWIDTH/PLATFORMTHICKNESS))+
  geom_boxplot()+
  facet_wrap(~RAWMATERIAL) +
  
  labs(fill = "",
       x = "Layers",
       y = "Platform Elongation") +
  theme_minimal()+
  theme_classic(base_size = 20)+
  scale_fill_viridis_d()+
  theme(axis.text.x = 
          element_text(colour = 
                         c(rep("skyblue",3),
                           rep("darkmagenta",2),
                           rep("forestgreen", 1)),
                       angle = 45, hjust = 1)) -> plat_elong1



df.flakes |>
  ggplot(aes(x = STRAT_mod, y = PLATFORMWIDTH/PLATFORMTHICKNESS))+
  geom_boxplot()+
  facet_wrap(~RAWMATERIAL) +
  ...
  theme(axis.text.x = 
          element_text(colour = 
                         c(rep("skyblue",1),
                           rep("darkmagenta",1),
                           rep("forestgreen", 1)),
                       angle = 45, hjust = 1)) -> plat_elong2
```

```{r}
png("../results/figures/fig_PlatElong.png", width = 290, height = 120, units = "mm", res = 300)
ggarrange(
  plat_elong2, plat_elong1,
  align = "h", labels = c("A", "B"),
  common.legend = TRUE
)
...
################### Flake elongation ######################
df.flakes |>
  ggplot(aes(x = STRAT, y = TECHLENGTH/TECHWIDTH))+
  geom_boxplot()+
  facet_wrap(~RAWMATERIAL) +
  ...
  theme(axis.text.x = 
          element_text(colour = 
                         c(rep("skyblue",3),
                           rep("darkmagenta",2),
                           rep("forestgreen", 1)),
                       angle = 45, hjust = 1)) -> flake_elong1


df.flakes |>
  ggplot(aes(x = STRAT_mod, y = TECHLENGTH/TECHWIDTH))+
  geom_boxplot()+
  facet_wrap(~RAWMATERIAL) +
  ...
  theme(axis.text.x = 
          element_text(colour = 
                         c(rep("skyblue",1),
                           rep("darkmagenta",1),
                           rep("forestgreen", 1)),
                       angle = 45, hjust = 1)) -> flake_elong2


################### Flake thinness ######################
df.flakes |>
  mutate(Thinness = TECHWIDTH/MAXTHICKNESS) |>
  filter(Thinness < 20) |>
  ggplot(aes(x = STRAT, y = Thinness))+
  geom_boxplot()+
  facet_wrap(~RAWMATERIAL) +
  ...
  theme(axis.text.x = 
          element_text(colour = 
                         c(rep("skyblue",3),
                           rep("darkmagenta",2),
                           rep("forestgreen", 1)),
                       angle = 45, hjust = 1)) -> flake_thin1


df.flakes |>
  mutate(Thinness = TECHWIDTH/MAXTHICKNESS) |>
  filter(Thinness < 20) |>
  ggplot(aes(x = STRAT_mod, y = Thinness))+
  geom_boxplot()+
  facet_wrap(~RAWMATERIAL) +
  ...
  theme(axis.text.x = 
          element_text(colour = 
                         c(rep("skyblue",1),
                           rep("darkmagenta",1),
                           rep("forestgreen", 1)),
                       angle = 45, hjust = 1)) -> flake_thin2
```

```{r}
png("../results/figures/fig_FlakeElong.png", width = 290, height = 120, units = "mm", res = 300)
ggarrange(
  plat_elong2, plat_elong1,
  align = "h", labels = c("A", "B"),
  common.legend = TRUE
...
```{r}
png("../results/figures/fig_FlakeThin.png", width = 290, height = 120, units = "mm", res = 300)
ggarrange(
  flake_thin2, flake_thin1,
  align = "h", labels = c("A", "B"),
  common.legend = TRUE
)
...
# Run Bayesian model with varying slopes
bayes_re2 <- brm(Utility_s ~ 1 + poly(Retouch_Intensity_s, 2) +
                   (0 + poly(Retouch_Intensity_s, 2) | STRAT/RAWMATERIAL),
              data = Bayes_red1,
              family = Gamma(link = "log"),
              iter = 2000,
              chains = 2,
...
bayes_re2_simp <- brm(Utility_s ~ 1 + Retouch_Intensity_s +
                   (0 + Retouch_Intensity_s | STRAT/RAWMATERIAL),
              data = Bayes_red1,
              family = Gamma(link = "log"),
              iter = 2000,
              chains = 2,
...
# Check posterior distirbutions and compare models
# simpler model (linear) fits best
loo_re1 <- loo(bayes_re2)
loo_simp <- loo(bayes_re2_simp)

loo_compare(loo_re1, loo_simp)


pp_check(bayes_re2_simp, type = "scatter_avg_grouped", group = "STRAT:RAWMATERIAL")
pp_check(bayes_re2, type = "scatter_avg_grouped", group = "STRAT:RAWMATERIAL")

pp_check(bayes_re2_simp)
pp_check(bayes_re2)


# Assign model to simpler
bayes_re2 <- bayes_re2_simp
```

```{r}
# set data to standardize the approach
Bayes_ut1 <- Bayes_red1
bayes_ut2 <- bayes_re2

x_seq <- seq(min(Bayes_ut1$Retouch_Intensity_s, na.rm = TRUE),
             max(Bayes_ut1$Retouch_Intensity_s, na.rm = TRUE),
             length.out = 100)
...
pred_summary <- newdat |>
  ...
  group_by(STRAT, RAWMATERIAL, Retouch_Intensity_s) |>
  summarise(
    mean = mean(Retouch_pred),
    lower = quantile(Retouch_pred, 0.025),
    upper = quantile(Retouch_pred, 0.975),
    .groups = "drop"
  )


ggplot(pred_summary, aes(x = Retouch_Intensity_s, y = mean, color = STRAT)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = lower, ymax = upper, fill = STRAT), alpha = 0.2, color = NA) +
  facet_wrap(~ RAWMATERIAL, scales = "free_y") +
  labs(
...
post <- spread_draws(
  ...
  # Layers
  `r_STRAT[1,Retouch_Intensity_s]`,
  `r_STRAT[2,Retouch_Intensity_s]`,
  `r_STRAT[3,Retouch_Intensity_s]`,
  `r_STRAT[4a,Retouch_Intensity_s]`,
  `r_STRAT[4b,Retouch_Intensity_s]`,
  
  # Quartz
  `r_STRAT:RAWMATERIAL[1_Quartz,Retouch_Intensity_s]`,
  `r_STRAT:RAWMATERIAL[2_Quartz,Retouch_Intensity_s]`,
  `r_STRAT:RAWMATERIAL[3_Quartz,Retouch_Intensity_s]`,
  `r_STRAT:RAWMATERIAL[4a_Quartz,Retouch_Intensity_s]`,
  `r_STRAT:RAWMATERIAL[4b_Quartz,Retouch_Intensity_s]`,

  
  # Quartzite
  `r_STRAT:RAWMATERIAL[2_Quartzite,Retouch_Intensity_s]`,
  `r_STRAT:RAWMATERIAL[3_Quartzite,Retouch_Intensity_s]`,
  `r_STRAT:RAWMATERIAL[4a_Quartzite,Retouch_Intensity_s]`,
  `r_STRAT:RAWMATERIAL[4b_Quartzite,Retouch_Intensity_s]`,
  
  
  # Silcrete
  `r_STRAT:RAWMATERIAL[1_Silcrete,Retouch_Intensity_s]`,
  `r_STRAT:RAWMATERIAL[2_Silcrete,Retouch_Intensity_s]`,
  `r_STRAT:RAWMATERIAL[3_Silcrete,Retouch_Intensity_s]`,
  `r_STRAT:RAWMATERIAL[4a_Silcrete,Retouch_Intensity_s]`,
  `r_STRAT:RAWMATERIAL[4b_Silcrete,Retouch_Intensity_s]`
...
slope_draws <- post |>
  ...
  mutate(
    ...
    # Quartz
    slope_Quartz_1 = b_Retouch_Intensity_s + 
      `r_STRAT[1,Retouch_Intensity_s]` +
      `r_STRAT:RAWMATERIAL[1_Quartz,Retouch_Intensity_s]`,
    slope_Quartz_2 = b_Retouch_Intensity_s + 
      `r_STRAT[2,Retouch_Intensity_s]` +
      `r_STRAT:RAWMATERIAL[2_Quartz,Retouch_Intensity_s]`,
    slope_Quartz_3 = b_Retouch_Intensity_s + 
      `r_STRAT[3,Retouch_Intensity_s]` +
      `r_STRAT:RAWMATERIAL[3_Quartz,Retouch_Intensity_s]`,
    slope_Quartz_4a = b_Retouch_Intensity_s + 
      `r_STRAT[4a,Retouch_Intensity_s]` +
      `r_STRAT:RAWMATERIAL[4a_Quartz,Retouch_Intensity_s]`,
    slope_Quartz_4b = b_Retouch_Intensity_s + 
      `r_STRAT[4b,Retouch_Intensity_s]` +
      `r_STRAT:RAWMATERIAL[4b_Quartz,Retouch_Intensity_s]`,
    
    # Quartzite
    slope_Quartzite_2 = b_Retouch_Intensity_s + 
      `r_STRAT[2,Retouch_Intensity_s]` +
      `r_STRAT:RAWMATERIAL[2_Quartzite,Retouch_Intensity_s]`,
    slope_Quartzite_3 = b_Retouch_Intensity_s + 
      `r_STRAT[3,Retouch_Intensity_s]` +
      `r_STRAT:RAWMATERIAL[3_Quartzite,Retouch_Intensity_s]`,
    slope_Quartzite_4a = b_Retouch_Intensity_s + 
      `r_STRAT[4a,Retouch_Intensity_s]` +
      `r_STRAT:RAWMATERIAL[4a_Quartzite,Retouch_Intensity_s]`,
    slope_Quartzite_4b = b_Retouch_Intensity_s + 
      `r_STRAT[4b,Retouch_Intensity_s]` +
      `r_STRAT:RAWMATERIAL[4b_Quartzite,Retouch_Intensity_s]`,
    
    
    # Silcrete
    slope_Silcrete_1 = b_Retouch_Intensity_s + 
      `r_STRAT[1,Retouch_Intensity_s]` +
      `r_STRAT:RAWMATERIAL[1_Silcrete,Retouch_Intensity_s]`,
    slope_Silcrete_2 = b_Retouch_Intensity_s + 
      `r_STRAT[2,Retouch_Intensity_s]` +
      `r_STRAT:RAWMATERIAL[2_Silcrete,Retouch_Intensity_s]`,
    slope_Silcrete_3 = b_Retouch_Intensity_s + 
      `r_STRAT[3,Retouch_Intensity_s]` +
      `r_STRAT:RAWMATERIAL[3_Silcrete,Retouch_Intensity_s]`,
    slope_Silcrete_4a = b_Retouch_Intensity_s + 
      `r_STRAT[4a,Retouch_Intensity_s]` +
      `r_STRAT:RAWMATERIAL[4a_Silcrete,Retouch_Intensity_s]`,
    slope_Silcrete_4b = b_Retouch_Intensity_s + 
      `r_STRAT[4b,Retouch_Intensity_s]` +
      `r_STRAT:RAWMATERIAL[4b_Silcrete,Retouch_Intensity_s]`,
...
slope_summary <- slope_draws |>
  select(c(starts_with("slope"))) |>
  pivot_longer(slope_Quartz_1:slope_Silcrete_4b, 
               names_to = "groups", values_to = "slopes") |>
  group_by(groups) |>
  summarise(
    mean = mean(slopes),
...
compare_slope |> select(starts_with("diff")) |>
  summarize(across(c(diff1:diff43), ~ mean(.x > 0)))



pairwise_diffs <- slope_draws |>
  select(c(starts_with("slope"))) |>
  pivot_longer(slope_Quartz_1:slope_Silcrete_4b, names_to = "groups", values_to = "slopes") |>
  group_by(groups) |>
  summarise(mean_slope = mean(slopes),
            lower = quantile(slopes, 0.025),
            upper = quantile(slopes, 0.975)) |>
...
# Probability that the slopes are less than 0
slope_draws |>
  summarize(across(slope_Quartz_1:slope_Silcrete_4b, ~ mean(.x < 0)))
```
...
flake.vr |>
  ...
  select(STRAT, STRAT_mod, RAWMATERIAL, V, VR, volume_removed, SDI, SA_Ellipse)-> cores.vr
```
...
Bayes_red1 <- lm_red |>
  mutate(V = V/1000,
         VR_s = scale(VR, center = T, scale = T)) |>
  select(V, VR_s, STRAT, STRAT_mod, RAWMATERIAL) |>
  na.omit()
...
# Run Bayesian model with varying slopes
bayes_core1 <- brm(V ~ 1 + poly(VR_s, 2) +
                   (0 + poly(VR_s, 2) | STRAT_mod/RAWMATERIAL),
              data = Bayes_red1,
              family = lognormal(),
              iter = 2000,
              chains = 2,
...
bayes_core1_simp <- brm(V ~ 1 + VR_s +
                   (0 + VR_s | STRAT_mod/RAWMATERIAL),
              data = Bayes_red1,
              family = lognormal(),
              iter = 5000,
              chains = 4,
...
# Check posterior distirbutions and compare models
# simpler model (linear) fits best
loo_core1 <- loo(bayes_core1)
loo_simp <- loo(bayes_core1_simp)

loo_compare(loo_core1, loo_simp)


pp_check(bayes_core1_simp, type = "scatter_avg_grouped", group = "STRAT_mod:RAWMATERIAL")
pp_check(bayes_core1, type = "scatter_avg_grouped", group = "STRAT_mod:RAWMATERIAL")

pp_check(bayes_core1_simp)
pp_check(bayes_core1)


# Assign model to more complex
```

```{r}
x_seq <- seq(min(Bayes_red1$VR_s, na.rm = TRUE),
             max(Bayes_red1$VR_s, na.rm = TRUE),
             length.out = 100)
...
# Get fitted values (posterior draws of the mean)
preds <- fitted(
  bayes_core1,       # your Gamma(log) model
  newdata = newdat,
  re_formula = NULL,    # include group-level effects
  summary = FALSE,       # return full posterior draws
  allow_new_levels = TRUE
...
pred_summary <- newdat |>
  bind_cols(as_tibble(t(preds))) |>
  rename("x" = "VR_s") |>
  pivot_longer(cols = starts_with("V"), values_to = "V_pred") |>
  rename("VR_s" = "x") |>
  group_by(STRAT_mod, RAWMATERIAL, VR_s) |>
  summarise(
    mean = mean(V_pred),
    lower = quantile(V_pred, 0.025),
    upper = quantile(V_pred, 0.975),
    .groups = "drop"
  )


ggplot(pred_summary, aes(x = VR_s, y = mean, color = STRAT_mod)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = lower, ymax = upper, fill = STRAT_mod), alpha = 0.2, color = NA) +
  facet_wrap(~ RAWMATERIAL, scales = "free_y") +
  labs(
...
post <- spread_draws(
  bayes_core1,
  b_Intercept,
  b_polyVR_s21, b_polyVR_s22,
  # Quartz
  `r_STRAT_mod:RAWMATERIAL[Coastal_Quartz,polyVR_s21]`,
  `r_STRAT_mod:RAWMATERIAL[Coastal_Quartz,polyVR_s22]`,
  
  `r_STRAT_mod:RAWMATERIAL[Mixed_Quartz,polyVR_s21]`,
  `r_STRAT_mod:RAWMATERIAL[Mixed_Quartz,polyVR_s22]`,
  
  `r_STRAT_mod:RAWMATERIAL[Terrestrial_Quartz,polyVR_s21]`,
  `r_STRAT_mod:RAWMATERIAL[Terrestrial_Quartz,polyVR_s22]`,
  
  # Quartzite
  `r_STRAT_mod:RAWMATERIAL[Coastal_Quartzite,polyVR_s21]`,
  `r_STRAT_mod:RAWMATERIAL[Coastal_Quartzite,polyVR_s22]`,
  
  `r_STRAT_mod:RAWMATERIAL[Mixed_Quartzite,polyVR_s21]`,
  `r_STRAT_mod:RAWMATERIAL[Mixed_Quartzite,polyVR_s22]`,
  
  `r_STRAT_mod:RAWMATERIAL[Terrestrial_Quartzite,polyVR_s21]`,
  `r_STRAT_mod:RAWMATERIAL[Terrestrial_Quartzite,polyVR_s22]`,
  
  # Silcrete
  `r_STRAT_mod:RAWMATERIAL[Coastal_Silcrete,polyVR_s21]`,
  `r_STRAT_mod:RAWMATERIAL[Coastal_Silcrete,polyVR_s22]`,
  
  `r_STRAT_mod:RAWMATERIAL[Mixed_Silcrete,polyVR_s21]`,
  `r_STRAT_mod:RAWMATERIAL[Mixed_Silcrete,polyVR_s22]`,
  
  `r_STRAT_mod:RAWMATERIAL[Terrestrial_Silcrete,polyVR_s21]`,
  `r_STRAT_mod:RAWMATERIAL[Terrestrial_Silcrete,polyVR_s22]`
)

# Determine the optimal surplus based on where tangent = 0
posterior_x <- post |>
  mutate(draw = row_number()) |>
  tidyr::crossing(VR_s = x_seq) |>
  mutate(
    # Quartz
    x_Quartz_C = (b_polyVR_s21 +
                         `r_STRAT_mod:RAWMATERIAL[Coastal_Quartz,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Coastal_Quartz,polyVR_s22]`)),
    
    x_Quartz_M = (b_polyVR_s21 +
                         `r_STRAT_mod:RAWMATERIAL[Mixed_Quartz,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Mixed_Quartz,polyVR_s22]`)),
    
    x_Quartz_T = (b_polyVR_s21 +
                         `r_STRAT_mod:RAWMATERIAL[Terrestrial_Quartz,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Terrestrial_Quartz,polyVR_s22]`)),
    
    # QUartzite
    x_Quartzite_C = (b_polyVR_s21 +
                         `r_STRAT_mod:RAWMATERIAL[Coastal_Quartzite,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Coastal_Quartzite,polyVR_s22]`)),
    
    x_Quartzite_M = (b_polyVR_s21 +
                         `r_STRAT_mod:RAWMATERIAL[Mixed_Quartzite,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Mixed_Quartzite,polyVR_s22]`)),
    
    x_Quartzite_T = (b_polyVR_s21 +
                         `r_STRAT_mod:RAWMATERIAL[Terrestrial_Quartzite,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Terrestrial_Quartzite,polyVR_s22]`)),
    
    # Silcrete
    x_Silcrete_C = (b_polyVR_s21 +
                         `r_STRAT_mod:RAWMATERIAL[Coastal_Silcrete,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Coastal_Silcrete,polyVR_s22]`)),
    
    x_Silcrete_M = (b_polyVR_s21 +
                         `r_STRAT_mod:RAWMATERIAL[Mixed_Silcrete,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Mixed_Silcrete,polyVR_s22]`)),
    
    x_Silcrete_T = (b_polyVR_s21 +
                         `r_STRAT_mod:RAWMATERIAL[Terrestrial_Silcrete,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Terrestrial_Silcrete,polyVR_s22]`)),
        ...
        # Quartz
    y_Quartz_C = ((b_polyVR_s21 +
                         `r_STRAT_mod:RAWMATERIAL[Coastal_Quartz,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Coastal_Quartz,polyVR_s22]`)*
              x_Quartz_C),
    
    y_Quartz_M = ((b_polyVR_s21 +
                      `r_STRAT_mod:RAWMATERIAL[Mixed_Quartz,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Mixed_Quartz,polyVR_s22]`)*
              x_Quartz_M),
    
    y_Quartz_T = ((b_polyVR_s21 +
                `r_STRAT_mod:RAWMATERIAL[Terrestrial_Quartz,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Terrestrial_Quartz,polyVR_s22]`)*
              x_Quartz_T),
    ...
    # Quartzite
    y_Quartzite_C = (b_polyVR_s21 +
                  `r_STRAT_mod:RAWMATERIAL[Coastal_Quartzite,polyVR_s21]`) +
            (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Coastal_Quartzite,polyVR_s22]`)*
              2*(x_Quartzite_C),
    
    y_Quartzite_M = ((b_polyVR_s21 +
                    `r_STRAT_mod:RAWMATERIAL[Mixed_Quartzite,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Mixed_Quartzite,polyVR_s22]`)*
              x_Quartzite_M),
    
    y_Quartzite_T = ((b_polyVR_s21 +
              `r_STRAT_mod:RAWMATERIAL[Terrestrial_Quartzite,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
              `r_STRAT_mod:RAWMATERIAL[Terrestrial_Quartzite,polyVR_s22]`)*
              x_Quartzite_T),
    ...
    # Silcrete
    y_Silcrete_C = ((b_polyVR_s21 +
                  `r_STRAT_mod:RAWMATERIAL[Coastal_Silcrete,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Coastal_Silcrete,polyVR_s22]`)*
              x_Silcrete_C),
    
    y_Silcrete_M = ((b_polyVR_s21 +
                    `r_STRAT_mod:RAWMATERIAL[Mixed_Silcrete,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Mixed_Silcrete,polyVR_s22]`)*
              x_Silcrete_M),
    
    y_Silcrete_T = ((b_polyVR_s21 +
              `r_STRAT_mod:RAWMATERIAL[Terrestrial_Silcrete,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
              `r_STRAT_mod:RAWMATERIAL[Terrestrial_Silcrete,polyVR_s22]`)*
              x_Silcrete_T)
...
slope_draws <- post |>
  ...
  mutate(
    ...
    # Quartz
    slope_Quartz_C = ((b_polyVR_s21 +
                         `r_STRAT_mod:RAWMATERIAL[Coastal_Quartz,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Coastal_Quartz,polyVR_s22]`)*
              VR_s),
    
    slope_Quartz_M = ((b_polyVR_s21 +
                      `r_STRAT_mod:RAWMATERIAL[Mixed_Quartz,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Mixed_Quartz,polyVR_s22]`)*
              VR_s),
    
    slope_Quartz_T = ((b_polyVR_s21 +
                `r_STRAT_mod:RAWMATERIAL[Terrestrial_Quartz,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Terrestrial_Quartz,polyVR_s22]`)*
              VR_s),
    ...
    # Quartzite
    slope_Quartzite_C = ((b_polyVR_s21 +
                  `r_STRAT_mod:RAWMATERIAL[Coastal_Quartzite,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Coastal_Quartzite,polyVR_s22]`)*
              VR_s),
    
    slope_Quartzite_M = ((b_polyVR_s21 +
                    `r_STRAT_mod:RAWMATERIAL[Mixed_Quartzite,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Mixed_Quartzite,polyVR_s22]`)*
              VR_s),
    
    slope_Quartzite_T = ((b_polyVR_s21 +
              `r_STRAT_mod:RAWMATERIAL[Terrestrial_Quartzite,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
              `r_STRAT_mod:RAWMATERIAL[Terrestrial_Quartzite,polyVR_s22]`)*
              VR_s),
    ...
    # Silcrete
    slope_Silcrete_C = ((b_polyVR_s21 +
                  `r_STRAT_mod:RAWMATERIAL[Coastal_Silcrete,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Coastal_Silcrete,polyVR_s22]`)*
              VR_s),
    
    slope_Silcrete_M = ((b_polyVR_s21 +
                    `r_STRAT_mod:RAWMATERIAL[Mixed_Silcrete,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT_mod:RAWMATERIAL[Mixed_Silcrete,polyVR_s22]`)*
              VR_s),
    
    slope_Silcrete_T = ((b_polyVR_s21 +
              `r_STRAT_mod:RAWMATERIAL[Terrestrial_Silcrete,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
              `r_STRAT_mod:RAWMATERIAL[Terrestrial_Silcrete,polyVR_s22]`)*
              VR_s)
...
# Summarize and visualize slopes
slope_summary <- slope_draws |>
  select(c(starts_with("slope"), VR_s)) |>
  pivot_longer(slope_Quartz_C:slope_Silcrete_T, 
               names_to = "groups", values_to = "slopes") |>
  group_by(groups, VR_s) |>
  summarise(
    mean = mean(slopes),
    lower = quantile(slopes, 0.025),
    upper = quantile(slopes, 0.975),
...
ggplot(slope_summary |> filter(groups %in% c("slope_Quartz_C",
                                             "slope_Quartz_M",
                                             "slope_Quartz_T")),
       aes(x = VR_s, y = mean, color = groups)) +
  geom_line() +
  geom_ribbon(aes(ymin = lower, ymax = upper, fill = groups), 
              alpha = 0.2, color = NA) +
  geom_hline(yintercept = 0, linetype = "dashed") +
...
compare_slope |>
  summarize(across(diff1:diff9, ~ mean(.x > 0)))
...
# Run Bayesian model with varying slopes
bayes_core1 <- brm(V ~ 1 + poly(VR_s, 2) +
                   (0 + poly(VR_s, 2) | STRAT/RAWMATERIAL),
              data = Bayes_red1,
              family = lognormal(),
              iter = 2000,
              chains = 2,
...
pp_check(bayes_core1_simp, type = "scatter_avg_grouped", group = "STRAT:RAWMATERIAL")
pp_check(bayes_core1, type = "scatter_avg_grouped", group = "STRAT:RAWMATERIAL")
...
pred_summary <- newdat |>
  ...
  pivot_longer(cols = starts_with("V"), values_to = "V_pred") |>
  rename("VR_s" = "x") |>
  group_by(STRAT, RAWMATERIAL, VR_s) |>
  summarise(
    mean = mean(V_pred),
    lower = quantile(V_pred, 0.025),
    upper = quantile(V_pred, 0.975),
...
ggplot(pred_summary, aes(x = VR_s, y = mean, color = STRAT)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = lower, ymax = upper, fill = STRAT), alpha = 0.2, color = NA) +
  facet_wrap(~ RAWMATERIAL, scales = "free_y") +
  labs(
...
post <- spread_draws(
  ...
  b_polyVR_s21, b_polyVR_s22,
  # Quartz
  `r_STRAT:RAWMATERIAL[1_Quartz,polyVR_s21]`,
  `r_STRAT:RAWMATERIAL[2_Quartz,polyVR_s21]`,
  `r_STRAT:RAWMATERIAL[3_Quartz,polyVR_s21]`,
  `r_STRAT:RAWMATERIAL[4a_Quartz,polyVR_s21]`,
  `r_STRAT:RAWMATERIAL[4b_Quartz,polyVR_s21]`,
  `r_STRAT:RAWMATERIAL[5_Quartz,polyVR_s21]`,
  
  `r_STRAT:RAWMATERIAL[1_Quartz,polyVR_s22]`,
  `r_STRAT:RAWMATERIAL[2_Quartz,polyVR_s22]`,
  `r_STRAT:RAWMATERIAL[3_Quartz,polyVR_s22]`,
  `r_STRAT:RAWMATERIAL[4a_Quartz,polyVR_s22]`,
  `r_STRAT:RAWMATERIAL[4b_Quartz,polyVR_s22]`,
  `r_STRAT:RAWMATERIAL[5_Quartz,polyVR_s22]`,

  
  # Quartzite
  `r_STRAT:RAWMATERIAL[1_Quartzite,polyVR_s21]`,
  `r_STRAT:RAWMATERIAL[2_Quartzite,polyVR_s21]`,
  `r_STRAT:RAWMATERIAL[3_Quartzite,polyVR_s21]`,
  `r_STRAT:RAWMATERIAL[4a_Quartzite,polyVR_s21]`,
  `r_STRAT:RAWMATERIAL[5_Quartzite,polyVR_s21]`,
  
  `r_STRAT:RAWMATERIAL[1_Quartzite,polyVR_s22]`,
  `r_STRAT:RAWMATERIAL[2_Quartzite,polyVR_s22]`,
  `r_STRAT:RAWMATERIAL[3_Quartzite,polyVR_s22]`,
  `r_STRAT:RAWMATERIAL[4a_Quartzite,polyVR_s22]`,
  `r_STRAT:RAWMATERIAL[5_Quartzite,polyVR_s22]`,
  
  
  # Silcrete
  `r_STRAT:RAWMATERIAL[3_Silcrete,polyVR_s21]`,
  `r_STRAT:RAWMATERIAL[4a_Silcrete,polyVR_s21]`,
  `r_STRAT:RAWMATERIAL[4b_Silcrete,polyVR_s21]`,
  `r_STRAT:RAWMATERIAL[5_Silcrete,polyVR_s21]`,
  
  `r_STRAT:RAWMATERIAL[3_Silcrete,polyVR_s22]`,
  `r_STRAT:RAWMATERIAL[4a_Silcrete,polyVR_s22]`,
  `r_STRAT:RAWMATERIAL[4b_Silcrete,polyVR_s22]`,
  `r_STRAT:RAWMATERIAL[5_Silcrete,polyVR_s22]`
)
...
posterior_x <- post |>
  ...
  mutate(
    # Quartz
    x_Quartz_1 = (b_polyVR_s21 +
                         `r_STRAT:RAWMATERIAL[1_Quartz,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[1_Quartz,polyVR_s22]`)),
    x_Quartz_2 = (b_polyVR_s21 +
                         `r_STRAT:RAWMATERIAL[2_Quartz,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[2_Quartz,polyVR_s22]`)),
    x_Quartz_3 = (b_polyVR_s21 +
                         `r_STRAT:RAWMATERIAL[3_Quartz,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[3_Quartz,polyVR_s22]`)),
    x_Quartz_4a = (b_polyVR_s21 +
                         `r_STRAT:RAWMATERIAL[4a_Quartz,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[4a_Quartz,polyVR_s22]`)),
    x_Quartz_4b = (b_polyVR_s21 +
                         `r_STRAT:RAWMATERIAL[4b_Quartz,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[4b_Quartz,polyVR_s22]`)),
    x_Quartz_5 = (b_polyVR_s21 +
                         `r_STRAT:RAWMATERIAL[5_Quartz,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[5_Quartz,polyVR_s22]`)),
    
    # QUartzite
    x_Quartzite_1 = (b_polyVR_s21 +
                         `r_STRAT:RAWMATERIAL[1_Quartzite,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[1_Quartzite,polyVR_s22]`)),
    x_Quartzite_2 = (b_polyVR_s21 +
                         `r_STRAT:RAWMATERIAL[2_Quartzite,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[2_Quartzite,polyVR_s22]`)),
    x_Quartzite_3 = (b_polyVR_s21 +
                         `r_STRAT:RAWMATERIAL[3_Quartzite,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[3_Quartzite,polyVR_s22]`)),
    x_Quartzite_4a = (b_polyVR_s21 +
                         `r_STRAT:RAWMATERIAL[4a_Quartzite,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[4a_Quartzite,polyVR_s22]`)),
    x_Quartzite_5 = (b_polyVR_s21 +
                         `r_STRAT:RAWMATERIAL[5_Quartzite,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[5_Quartzite,polyVR_s22]`)),
    ...
    # Silcrete
    x_Silcrete_3 = (b_polyVR_s21 +
                         `r_STRAT:RAWMATERIAL[3_Silcrete,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[3_Silcrete,polyVR_s22]`)),
    x_Silcrete_4a = (b_polyVR_s21 +
                         `r_STRAT:RAWMATERIAL[4a_Silcrete,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[4a_Silcrete,polyVR_s22]`)),
    x_Silcrete_4b = (b_polyVR_s21 +
                         `r_STRAT:RAWMATERIAL[4b_Silcrete,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[4b_Silcrete,polyVR_s22]`)),
    x_Silcrete_5 = (b_polyVR_s21 +
                         `r_STRAT:RAWMATERIAL[5_Silcrete,polyVR_s21]`)/
            -1*(2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[5_Silcrete,polyVR_s22]`)),
        ...
        # Quartz
    y_Quartz_1 = ((b_polyVR_s21 +
                         `r_STRAT:RAWMATERIAL[1_Quartz,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[1_Quartz,polyVR_s22]`)*
              x_Quartz_1),
    y_Quartz_2 = ((b_polyVR_s21 +
                         `r_STRAT:RAWMATERIAL[2_Quartz,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[2_Quartz,polyVR_s22]`)*
              x_Quartz_2),
    y_Quartz_3 = ((b_polyVR_s21 +
                         `r_STRAT:RAWMATERIAL[3_Quartz,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[3_Quartz,polyVR_s22]`)*
              x_Quartz_3),
    y_Quartz_4a = ((b_polyVR_s21 +
                         `r_STRAT:RAWMATERIAL[4a_Quartz,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[4a_Quartz,polyVR_s22]`)*
              x_Quartz_4a),
    y_Quartz_4b = ((b_polyVR_s21 +
                         `r_STRAT:RAWMATERIAL[4b_Quartz,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[4b_Quartz,polyVR_s22]`)*
              x_Quartz_4b),
    y_Quartz_5 = ((b_polyVR_s21 +
                         `r_STRAT:RAWMATERIAL[5_Quartz,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[5_Quartz,polyVR_s22]`)*
              x_Quartz_5),
    ...
    y_Quartzite_2 = (b_polyVR_s21 +
                  `r_STRAT:RAWMATERIAL[2_Quartzite,polyVR_s21]`) +
            (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[2_Quartzite,polyVR_s22]`)*
              2*(x_Quartzite_2),
    y_Quartzite_3 = (b_polyVR_s21 +
                  `r_STRAT:RAWMATERIAL[3_Quartzite,polyVR_s21]`) +
            (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[3_Quartzite,polyVR_s22]`)*
              2*(x_Quartzite_3),
    y_Quartzite_4a = (b_polyVR_s21 +
                  `r_STRAT:RAWMATERIAL[4a_Quartzite,polyVR_s21]`) +
            (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[4a_Quartzite,polyVR_s22]`)*
              2*(x_Quartzite_4a),
    y_Quartzite_5 = (b_polyVR_s21 +
                  `r_STRAT:RAWMATERIAL[5_Quartzite,polyVR_s21]`) +
            (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[5_Quartzite,polyVR_s22]`)*
              2*(x_Quartzite_5),
    ...
    # Silcrete
    y_Silcrete_3 = ((b_polyVR_s21 +
                  `r_STRAT:RAWMATERIAL[3_Silcrete,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[3_Silcrete,polyVR_s22]`)*
              x_Silcrete_3),
    y_Silcrete_4a = ((b_polyVR_s21 +
                  `r_STRAT:RAWMATERIAL[4a_Silcrete,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[4a_Silcrete,polyVR_s22]`)*
              x_Silcrete_4a),
    y_Silcrete_4b = ((b_polyVR_s21 +
                  `r_STRAT:RAWMATERIAL[4b_Silcrete,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[4b_Silcrete,polyVR_s22]`)*
              x_Silcrete_4b),
    y_Silcrete_5 = ((b_polyVR_s21 +
                  `r_STRAT:RAWMATERIAL[5_Silcrete,polyVR_s21]`) +
            2 * (b_polyVR_s22 +
                   `r_STRAT:RAWMATERIAL[5_Silcrete,polyVR_s22]`)*
              x_Silcrete_5)
...
cores.vr |>
  ...
  summarize(volume_ave = mean(V),
            ...
            volumeRec_sd = sd(volume_removed)) |>
  
  ggplot(aes(x = volumeRec_mean, y = log(volume_ave),
             color = STRAT, label = STRAT))+
  geom_point(size = 2)+
  geom_ellipse(aes(x0 = volumeRec_mean, y0 = log(volume_ave),
                   b = log(volume_sd), a= volumeRec_sd,
                   angle = 0))+
  geom_text(aes(fontface = 2),hjust = -0.2, vjust = -0.5)+
...
scrap.red <- scrap.red |>
  mutate(RETOUCH = as.vector(scrap.red$Reduction),
         ...
         STRAT_mod = df.scrap$STRAT_mod,
         UTILITY_small = df.scrap$Utility_small,
...
Bayes_red1 <- lm_red |>
  mutate(RETOUCH_s = scale(RETOUCH, center = T, scale = T)) |>
  select(RETOUCH_s, UTILITY_small, STRAT, STRAT_mod, RAWMATERIAL) |>
  na.omit()
...
# Run Bayesian model with varying slopes
bayes_core1 <- brm(UTILITY_small ~ 1 + poly(RETOUCH_s, 2) +
                   (0 + poly(RETOUCH_s, 2) | STRAT_mod/RAWMATERIAL),
              data = Bayes_red1,
              family = lognormal(),
              iter = 2000,
              chains = 2,
...
bayes_core1_simp <- brm(UTILITY_small ~ 1 + RETOUCH_s +
                   (0 + RETOUCH_s | STRAT_mod/RAWMATERIAL),
              data = Bayes_red1,
              family = lognormal(),
              iter = 2000,
              chains = 2,
...
# Assign model to more simple
bayes_re1 <- bayes_core1_simp
```

```{r}
x_seq <- seq(min(Bayes_red1$RETOUCH_s, na.rm = TRUE),
             max(Bayes_red1$RETOUCH_s, na.rm = TRUE),
             length.out = 100)
...
pred_summary <- newdat |>
  bind_cols(as_tibble(t(preds))) |>
  pivot_longer(cols = starts_with("V"), values_to = "Retouch_pred") |>
  group_by(STRAT_mod, RAWMATERIAL, RETOUCH_s) |>
  summarise(
    mean = mean(Retouch_pred),
    lower = quantile(Retouch_pred, 0.025),
    upper = quantile(Retouch_pred, 0.975),
...
ggplot(pred_summary, aes(x = RETOUCH_s, y = mean, color = STRAT_mod)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = lower, ymax = upper, fill = STRAT_mod), alpha = 0.2, color = NA) +
  facet_wrap(~ RAWMATERIAL, scales = "free_y") +
  labs(
...
post <- spread_draws(
  ...
  # Coastal
  `r_STRAT_mod[Coastal,RETOUCH_s]`,
  # Mixed
  `r_STRAT_mod[Mixed,RETOUCH_s]`,
  # Terrestrial
  `r_STRAT_mod[Terrestrial,RETOUCH_s]`,
  
  # Quartz
  `r_STRAT_mod:RAWMATERIAL[Coastal_Quartz,RETOUCH_s]`,
  `r_STRAT_mod:RAWMATERIAL[Mixed_Quartz,RETOUCH_s]`,
  
  # Silcrete
  `r_STRAT_mod:RAWMATERIAL[Coastal_Silcrete,RETOUCH_s]`,
  `r_STRAT_mod:RAWMATERIAL[Mixed_Silcrete,RETOUCH_s]`,
  `r_STRAT_mod:RAWMATERIAL[Terrestrial_Silcrete,RETOUCH_s]`,
)
```
...
slope_draws <- post |>
  ...
  mutate(
    ...
    # Quartz
    slope_Quartz_C = b_RETOUCH_s + 
      `r_STRAT_mod[Coastal,RETOUCH_s]` +
      `r_STRAT_mod:RAWMATERIAL[Coastal_Quartz,RETOUCH_s]`,
    
    slope_Quartz_M = b_RETOUCH_s + 
      `r_STRAT_mod[Mixed,RETOUCH_s]` +
      `r_STRAT_mod:RAWMATERIAL[Mixed_Quartz,RETOUCH_s]`,
    
    
    # Silcrete
    slope_Silcrete_C = b_RETOUCH_s + 
      `r_STRAT_mod[Coastal,RETOUCH_s]` +
      `r_STRAT_mod:RAWMATERIAL[Coastal_Silcrete,RETOUCH_s]`,
    
    slope_Silcrete_M = b_RETOUCH_s + 
      `r_STRAT_mod[Mixed,RETOUCH_s]` +
      `r_STRAT_mod:RAWMATERIAL[Mixed_Silcrete,RETOUCH_s]`,
    
    slope_Silcrete_T = b_RETOUCH_s + 
      `r_STRAT_mod[Terrestrial,RETOUCH_s]` +
      `r_STRAT_mod:RAWMATERIAL[Terrestrial_Silcrete,RETOUCH_s]`
  )
...
compare_slope <- slope_draws |>
  mutate(diff1 = slope_Quartz_C - slope_Quartz_M,
         ...
         diff2 = slope_Silcrete_C - slope_Silcrete_M,
         diff3 = slope_Silcrete_C - slope_Silcrete_T,
         diff4 = slope_Silcrete_M - slope_Silcrete_T)

compare_slope |>
  summarize(across(diff1:diff4, ~ mean(.x > 0))) |>
  print()
...
# Probability that the slopes are less than 0
slope_draws |>
  summarize(across(slope_Quartz_C:slope_Silcrete_T, ~ mean(.x < 0)))
```

```{r}
compare_slope |>
  select(starts_with("diff")) |>
  rename("Quartz (Coastal - Mixed)" = "diff1",
         "Silcrete (Coastal - Mixed)" = "diff2",
         "Silcrete (Coastal - Terrestrial)" = "diff3",
         "Silcrete (Mixed - Terrestrial)" = "diff4") |>
  pivot_longer(`Quartz (Coastal - Mixed)`:`Silcrete (Mixed - Terrestrial)`,
               names_to = "layer", values_to = "slope") |>
  ...
  labs(x = "Posterior Slope Estimate", y = "Raw material") +
...
# Run Bayesian model with varying slopes
bayes_core1 <- brm(UTILITY_small ~ 1 + poly(RETOUCH_s, 2) +
                   (0 + poly(RETOUCH_s, 2) | STRAT/RAWMATERIAL),
              data = Bayes_red1,
              family = lognormal(),
              iter = 2000,
              chains = 2,
...
# Get fitted values (posterior draws of the mean)
preds <- fitted(
  bayes_re2,       # your Gamma(log) model
  newdata = newdat,
  re_formula = NULL,    # include group-level effects
  summary = FALSE,       # return full posterior draws
  allow_new_levels = TRUE
...
pred_summary <- newdat |>
  bind_cols(as_tibble(t(preds))) |>
  pivot_longer(cols = starts_with("V"), values_to = "Retouch_pred") |>
  group_by(STRAT, RAWMATERIAL, RETOUCH_s) |>
  summarise(
    mean = mean(Retouch_pred),
    lower = quantile(Retouch_pred, 0.025),
    upper = quantile(Retouch_pred, 0.975),
...
ggplot(pred_summary, aes(x = RETOUCH_s, y = mean, color = STRAT)) +
  geom_line(size = 1) +
  geom_ribbon(aes(ymin = lower, ymax = upper, fill = STRAT), alpha = 0.2, color = NA) +
  facet_wrap(~ RAWMATERIAL, scales = "free_y") +
  labs(
...
post <- spread_draws(
  ...
  `r_STRAT[2,RETOUCH_s]`,
  `r_STRAT[3,RETOUCH_s]`,
  `r_STRAT[4a,RETOUCH_s]`,
  `r_STRAT[4b,RETOUCH_s]`,
  `r_STRAT[5,RETOUCH_s]`,
  
  # Quartz
  `r_STRAT:RAWMATERIAL[1_Quartz,RETOUCH_s]`,
  `r_STRAT:RAWMATERIAL[2_Quartz,RETOUCH_s]`,
  `r_STRAT:RAWMATERIAL[3_Quartz,RETOUCH_s]`,
  `r_STRAT:RAWMATERIAL[4a_Quartz,RETOUCH_s]`,
  `r_STRAT:RAWMATERIAL[4b_Quartz,RETOUCH_s]`,
  
  
  # Silcrete
  `r_STRAT:RAWMATERIAL[2_Silcrete,RETOUCH_s]`,
  `r_STRAT:RAWMATERIAL[4a_Silcrete,RETOUCH_s]`,
  `r_STRAT:RAWMATERIAL[4b_Silcrete,RETOUCH_s]`,
  `r_STRAT:RAWMATERIAL[5_Silcrete,RETOUCH_s]`
...
slope_draws <- post |>
  ...
  mutate(
    ...
    # Quartz
    slope_Quartz_1 = b_RETOUCH_s + 
      `r_STRAT[1,RETOUCH_s]` +
      `r_STRAT:RAWMATERIAL[1_Quartz,RETOUCH_s]`,
    slope_Quartz_2 = b_RETOUCH_s + 
      `r_STRAT[2,RETOUCH_s]` +
      `r_STRAT:RAWMATERIAL[2_Quartz,RETOUCH_s]`,
    slope_Quartz_3 = b_RETOUCH_s + 
      `r_STRAT[3,RETOUCH_s]` +
      `r_STRAT:RAWMATERIAL[3_Quartz,RETOUCH_s]`,
    slope_Quartz_4a = b_RETOUCH_s + 
      `r_STRAT[4a,RETOUCH_s]` +
      `r_STRAT:RAWMATERIAL[4a_Quartz,RETOUCH_s]`,
    slope_Quartz_4b = b_RETOUCH_s + 
      `r_STRAT[4b,RETOUCH_s]` +
      `r_STRAT:RAWMATERIAL[4b_Quartz,RETOUCH_s]`,
    
    
    # Silcrete
    slope_Silcrete_2 = b_RETOUCH_s + 
      `r_STRAT[2,RETOUCH_s]` +
      `r_STRAT:RAWMATERIAL[2_Silcrete,RETOUCH_s]`,
    slope_Silcrete_4a = b_RETOUCH_s + 
      `r_STRAT[4a,RETOUCH_s]` +
      `r_STRAT:RAWMATERIAL[4a_Silcrete,RETOUCH_s]`,
    slope_Silcrete_4b = b_RETOUCH_s + 
      `r_STRAT[4b,RETOUCH_s]` +
      `r_STRAT:RAWMATERIAL[4b_Silcrete,RETOUCH_s]`,
    slope_Silcrete_5 = b_RETOUCH_s + 
      `r_STRAT[5,RETOUCH_s]` +
      `r_STRAT:RAWMATERIAL[5_Silcrete,RETOUCH_s]`
...
compare_slope <- slope_draws |>
  mutate(# Quartz comparisons
         ...
         diff40= slope_Silcrete_4a - slope_Silcrete_4b,
         diff42= slope_Silcrete_4a - slope_Silcrete_5,
         
         diff43= slope_Silcrete_4b - slope_Silcrete_5
...
# Probability that the slopes are less than 0
slope_draws |>
  summarize(across(slope_Quartz_1:slope_Silcrete_5, ~ mean(.x < 0)))
```
...
scrap.red |>
  ...
  summarize(Retouch_ave = mean(RETOUCH, na.rm = T),
            Retouch_sd = sd(RETOUCH, na.rm = T),
            
            Utility_ave = mean(UTILITY_small, na.rm = T),
            Utility_sd = sd(UTILITY_small, na.rm = T)) |>
  
  ggplot(aes(x = Retouch_ave, y = Utility_ave, 
             color = STRAT, label = STRAT))+
  geom_point(size = 3)+
  geom_ellipse(aes(x0 = Retouch_ave, y0 = Utility_ave,
                   a = Retouch_sd, b = Utility_sd,
                   angle = 0))+
  facet_grid(~RAWMATERIAL)+
