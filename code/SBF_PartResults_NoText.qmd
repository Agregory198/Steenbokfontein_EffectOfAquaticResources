...
Bayes_red1 <- Bayes_red1[-c(619, 1334, 1478, 1533),]
Bayes_red1 <- Bayes_red1[-c(625, 1479),]
...
```{r junk}
#| include: false
bayesian_viz <- function(BayesModel, Data, StratGroup){
  
  # Define list to store plots
  plot_list <- list()
  
  x_seq <- seq(min(Data$Reduction_s, na.rm = TRUE),
             max(Data$Reduction_s, na.rm = TRUE),
             length.out = 100)
  
  # All combinations of grouping levels
  newdat <- expand_grid(
    Reduction_s = x_seq,
    STRAT_mod = unique(Data$StratGroup),
    RAWMATERIAL = unique(Data$RAWMATERIAL)
  )
  
  preds <- fitted(
    BayesModel,       # your Gamma(log) model
    newdata = newdat,
  ...
  plot1 <- ggplot(pred_summary, aes(x = Reduction_s, y = mean, color = STRAT_mod)) +
  geom_line(size = 1) +
  ...
  plot_list[["Utility_Reduction_Curve"]] <- plot1
  
  
  ###################################################
  # Assign sequence to procude slope values
  x_seq <- seq(min(Data$Reduction_s),
               max(Data$Reduction_s),
               length.out = 50)
